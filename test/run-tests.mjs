...
expectNoRule('placeholder', 'カテゴリ変数からダミー変数を作成した。');
expectNoRule('placeholder', '性別ダミー、地域ダミー、年度ダミーを説明変数に加えた。');
expectNoRule('placeholder', '性別ダミー、地域ダミー、年度ダミー、学年ダミーを回帰モデルに加えた。');
expectRule('placeholder', '公開前の画面にはダミーを入れておく。');
expectRule('placeholder', '公開前にダミー変数を作成した。');
...
assert.equal(findingsFor('placeholder', '本番値の代替としてダミー変数を説明変数欄に入力しておく。').length, 1);
assert.equal(findingsFor('placeholder', '本番値の代わりとしてダミー変数を説明変数欄に入力しておく。').length, 1);
const mixedDummyClauseText = '公開前の画面にはダミーと表示し性別ダミーを説明変数に加えた。';
const mixedDummyClauseFindings = findingsFor('placeholder', mixedDummyClauseText);
assert.equal(mixedDummyClauseFindings.length, 1, 'only the UI placeholder dummy should be detected');
assert.equal(mixedDummyClauseFindings[0].index, mixedDummyClauseText.indexOf('ダミー'), 'the first dummy should be detected');
assert.equal(findingsFor('placeholder', 'ダミー（v1.0で削除予定）を説明変数欄に入力した。').length, 1);
assert.equal(findingsFor('placeholder', '公開前のフォームにはダミー項番を残した。').length, 1);
assert.equal(findingsFor('placeholder', '公開前のフォームにはダミー項名を残した。').length, 1);
expectNoRule('placeholder', '処置群ダミー（1なら処置群、0なら対照群）の係数を推定した。');
expectNoRule('placeholder', '処置群ダミー（処置群=1、対照群=0）を回帰モデルに加えた。');
expectNoRule('placeholder', '性別ダミーと地域ダミーを説明変数に加えた。');
expectNoRule('placeholder', '性別ダミー、地域ダミー、年度ダミーを説明変数に加えた。');
expectNoRule('placeholder', '回帰式にダミー項を加えた。');
expectNoRule('placeholder', 'ダミー項の係数を推定した。');
